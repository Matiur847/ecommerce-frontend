...
export const getAllUsers = createAsyncThunk(
  ...
  async (order) => {
    try {
      ...
      const { data } = await axios.get("https://ecommerce-backend-tzi7.onrender.com/api/v1/admin/users");
      return data;
    } catch (error) {
      throw error;
    }
...
export const userDetailAdmin = createAsyncThunk(
  ...
  async (id) => {
    try {
      // const config = { Headers: { "Content-Type": "application/json" } };
      const { data } = await axios.get(`https://ecommerce-backend-tzi7.onrender.com/api/v1/admin/user/${id.id}`);
      return data;
    } catch (error) {
      throw error;
    }
  ...
  async (allData) => {
    try {
      ...
      const { data } = await axios.put(
        `https://ecommerce-backend-tzi7.onrender.com/api/v1/admin/update/user/role/${id.id}`,
        myForm,
        config
      );
      return data;
...
export const userDeleteAdmin = createAsyncThunk(
  ...
  async (id) => {
    try {
      // const config = { Headers: { "Content-Type": "application/json" } };
      const { data } = await axios.delete(`https://ecommerce-backend-tzi7.onrender.com/api/v1/admin/user/delete/${id}`);
      return data;
    } catch (error) {
      throw error;
    }
